...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        ####### 계좌 관련된 변수
        self.account_stock_dict = {} # 현재 보유하고 있는 종목 dict
        self.not_account_stock_dict = {}
        self.account_num = None #계좌번호 담아줄 변수
        self.secret_num = None # 계좌비밀번호 담아줄 변수

        self.deposit = 0 #예수금
        self.use_money = 0 #종목별 실제 투자에 사용할 금액
        self.use_money_percent = 0.5 #예수금에서 실제 사용할 비율
        self.output_deposit = 0 #출금 가능 금액

        self.total_buy_money = 0 # 총 매입 금액
        self.total_asset = 0 # 예수금 포함 추정 자산
    ...
    def detail_account_info(self, sPrevNext="0"):
        try:
            self.secret_num = self.myInfo.account[self.account_num]
        except:
            self.secret_num = '0000'

        self.dynamicCall("SetInputValue(QString, QString)", "계좌번호", self.account_num)
        self.dynamicCall("SetInputValue(QString, QString)", "비밀번호", self.secret_num)
    ...
    def detail_account_mystock(self, sPrevNext="0"):
        ...
        self.dynamicCall("SetInputValue(QString, QString)", "비밀번호입력매체구분", "00")
        self.dynamicCall("SetInputValue(QString, QString)", "조회구분", "1")
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "계좌평가잔고내역요청", "opw00018", sPrevNext, self.screen_my_info) # sRQName, sTrCode, nPrevNext, sScreenNo
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            total_asset = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, 0, "추정예탁자산") # 출력 : 000000000746100
            self.total_asset = int(total_asset)
            ...
            self.logging.logger.debug("계좌평가잔고내역요청 싱글데이터 : 총매입금액 %s - 추정예탁자산 %s - 총평가손익금액 %s - 총수익률 %s" %
                                      (self.total_buy_money, self.total_asset, self.total_profit_loss_money, self.total_profit_loss_rate))
